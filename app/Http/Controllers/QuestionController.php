...
class QuestionController extends Controller
{
	...
	public function randSoal(){
		...
		DB::update('insert into tr_question(soal_id,user_id,question,first_opt,second_opt,third_opt,forth_opt,answer,mod_soal,answer_user,time_close) select id, ? ,question,first_option, second_option,third_option, forth_option,answer, 10,0,DATE_ADD(now(), INTERVAL +2 HOUR) from questions order by rand() limit 20',[$userId]);
		return redirect('edit/1');
	}
	public function score(){
		$scr = DB::select("select sum(hasil) as total_score from(
							...
							from tr_question where user_id = 'bambang' and answer_user != '0') asd");
